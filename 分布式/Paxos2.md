...
在论文中，兰伯特没有说如何选举领导者，需要我们在实现 Multi-Paxos 算法的时候自己实现。（Chubby）实现中，是通过执行Basic Paxos算法，进行投票选举产生。
