...
但是Android要求主线程：
线程启动 －－ 持续等待事件 －－ 处理事件 －－ 继续等待 －－ 永不退出。

Looper与MessageQueue、Handler、Message支撑起这一模型。一个Looper对应一个MessageQueue对应多个Handler。

Looper的核心是以线程为Key的Map，主线程的Looper单独保存一份引用；每个线程持有一个MessageQueue（Android17引入无锁队列，可能会有变化）；Looper所属线程也会被记录。loop方法是核心的事件循环，在loop中会不停地调用loopOnce，从MessageQueue中获取下一条消息，会有各种trace，重点是把消息分发给对应的Handler。

## MessageQueue

MessageQueue本质是一个按照时间排序的优先级队列，单向链表实现，通过epoll＋eventfd实现阻塞与唤醒。

Native层立面包含了epoll相关的代码，工作流程类似于：

线程A发送消息，通过write eventfd给到线程B，线程B通过epoll wait epollfd等待消息，然后read。

MessageQueue核心是enqueMessage和next，next中会执行nativePollOnce，检查链表头是否是同步屏障，根据不同分支执行不同流程，但最终都会通过continue回到nativePoll。
