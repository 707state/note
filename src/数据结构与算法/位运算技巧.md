...
- [869 重新排序得到2的幂](#869-重新排序得到2的幂)
- [UNSOLVED 3495 使数组元素都变为零的最少操作次数](#unsolved-3495-使数组元素都变为零的最少操作次数)
- [Meet In The Middle技巧](#meet-in-the-middle技巧)
  - [2035 将数组分成两个数组并最小化数组和的差](#2035-将数组分成两个数组并最小化数组和的差)
- [UNSOLVED 3133 数组最后一个元素的最小值](#unsolved-3133-数组最后一个元素的最小值)
<!--toc:end-->
...
class Solution {
    ...
    bool backtracking(string& nums,int idx,int num){
        if(idx==nums.length()){
        ...
        for(int i=0;i<nums.length();i++){
            ...
            vis[i]=1;
            if(backtracking(nums,idx+1,num*10+nums[i]-'0')){
                return true;
            }
            vis[i]=0;
        }
...
# UNSOLVED 3495 使数组元素都变为零的最少操作次数
...
# Meet In The Middle技巧

「Meet in the middle」是一个搜索技巧，可以在输入很小但是却不足以小到可以使用暴力法算时使用。有点像「分治」，把原始集合划分为两个相等的集合，分别解决，然后合并，但又不完全是「分治」，因为划分后的集合和源集合没有相同的子结构。
一般过程如下：

将整数集合划分为A、B两个集合。A、B各含有源集合的一半；
分别求出A、B中所有子集的和，并将结果保存在X和Y（可以使用位运算来枚举子集来实现求和）；
然后开始合并两个子问题：找到X和Y的组合中小于等于「目标值」（这里是数组和的一半，因为当两部和越接近数组和的一半时，差最小）的组合。
（组合X、Y时，可以先将Y排序，然后对X的每一个值在Y中进行二分查找，可以将组合时间复杂度降低到O(nlogn))

总结：「Meet in the middle」可以用来用来求「数组中取任意个数，使其最近接目标值的问题」。
...
这道题目的核心就是找到一个有n个元素的集合B使得abs(total-2*sum(B))的值最小。也就是2*sum(B)的值最接近total。
...
public:
    int minimumDifference(vector<int>& nums) {
        ...
        for(int mask=0;mask<1<<n;mask++){
            int key=__builtin_popcount(mask);
            int cnt_a=0,cnt_b=0;
            for(int i=0;i<n;i++){
                if((mask>>i)&1){
                    cnt_a+=a[i];
            ...
            x[key].push_back(cnt_a);
            y[key].push_back(cnt_b);
        }
        // 方便排序
        for(int i=0;i<=n;i++){
            ranges::sort(y[i]);
        ...
        for(int k=0;k<=n;k++){
            ...
            for(auto i:x_i){
                ...
                ans=min(ans,abs((total-2*(i+y_i[l]))));
            }
        }
        return ans;
    }
