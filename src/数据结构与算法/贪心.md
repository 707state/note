...
public:
    int minimumTeachings(int n, vector<vector<int>>& languages, vector<vector<int>>& friendships) {
        unordered_set<int> friends_cant_talk;
        for(auto& friendship: friendships){
            unordered_map<int,int> common_language;
            for(auto i: languages[friendship[0]-1]){
                common_language[i]++;
            }
            bool has_common_language=false;
            for(auto j: languages[friendship[1]-1]){
                if(common_language[j]){
                    has_common_language=true;
                    break;
                }
            }
            if(!has_common_language){
                friends_cant_talk.insert(friendship[0]);
                friends_cant_talk.insert(friendship[1]);
            }
        }
        unordered_map<int,int> cnt;
        int max_common_langauges=0;
        for(const auto person: friends_cant_talk){
            for(auto l: languages[person-1]){
                cnt[l]++;
                max_common_langauges=max(max_common_langauges,cnt[l]);
            }
        }
        return friends_cant_talk.size()-max_common_langauges;
    }
};
```
