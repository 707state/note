...
- [1621 大小为 K 的不重叠线段的数目](#1621-大小为k的不重叠线段的数目)
- [312 戳气球](#312-戳气球)
  - [记忆化搜索](#记忆化搜索)
  - [动态规划](#动态规划)
...
从右向左计算 dp 值(从后向前)，当前位置如果为 i
则它如果直接跳到右边（前面）去就是 dp\[jump\[i\]+i\]+1（这个值已经计算过了），计算出当前位置 dp\[i\]之后，当前位置 i 可以影响
i+1 到 dp\[j\] \>=
dp\[i\]+1 位置上的值（因为某个位置可以跳到左边任意位置）注意遍历到 dp\[j\]\>=dp\[i\]+1 即可。
