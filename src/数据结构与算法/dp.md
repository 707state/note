...
public:
    int uniquePaths(vector<vector<int>>& grid) {
        ...
        vector<vector<array<int,2>>> dp(m, vector<array<int,2>>(n,{0,0}));

        // 起点，从 (0,0) 可以尝试往右或往下走
        if(n > 1) dp[0][1][0] = 1; // 从左进入 (0,1)
        if(m > 1) dp[1][0][1] = 1; // 从上进入 (1,0)
        ...
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                for(int d=0; d<2; d++){
                    ...
                    if(grid[i][j]==0){
                        // 空格子，能继续往右、下
                        if(j+1<n) dp[i][j+1][0] = (dp[i][j+1][0] + ways) % MOD;
                        if(i+1<m) dp[i+1][j][1] = (dp[i+1][j][1] + ways) % MOD;
                    }else{
                        // 镜子格子：根据进入方向反射
                        if(d==0){ // 从左进入 → 转向下
                            if(i+1<m) dp[i+1][j][1] = (dp[i+1][j][1] + ways) % MOD;
