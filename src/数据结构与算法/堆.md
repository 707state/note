...
public:
    ...
    // push_heap: 将新元素加入堆并维护堆性质
    void push_heap(int value) {
        arr.push_back(value);
        sift_up(arr.size() - 1);
    }

    // pop_heap: 移除堆顶元素并维护堆性质
    int pop_heap() {
        if (arr.empty()) {
    ...
    // 构造函数
    Heap() = default;
    Heap(const std::vector<int>& initial_arr) : arr(initial_arr) {
        build_heap();
    }
...
// 独立的函数版本（类似STL风格）
void push_heap(std::vector<int>& arr, int value) {
    arr.push_back(value);
...
int pop_heap(std::vector<int>& arr) {
    if (arr.empty()) {
        throw std::runtime_error("Heap is empty");
    }
...
// 测试代码
int main() {
    std::cout << "=== 类版本测试 ===" << std::endl;
    Heap heap({2, 6, 1, 4, 3, 9, 7});
    heap.print();
    ...
    std::cout << "Pop: " << heap.pop_heap() << std::endl;
    heap.print();
    ...
    std::cout << "\n=== 函数版本测试 ===" << std::endl;
    std::vector<int> arr{2, 6, 1, 4, 3, 9, 7};
    ...
    std::cout << "Pop: " << pop_heap(arr) << std::endl;
    std::cout << "After pop: [ ";
    for (int num : arr) std::cout << num << " ";
    std::cout << "]" << std::endl;
